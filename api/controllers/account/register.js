    ...
    if (createdUser) {
      ...
      const mailOptions = {
        ...
        html: `
                ...
                <p>To activate your account click on <a href="http://${clientServer}/registration/${
          finalUser.id
        }/${emailToken}">link</a> .</p>
                Thanks<br>
      ...
      transporter.sendMail(mailOptions, function(err, info) {
        ...
        else console.log("mail sent sucessfully", info);
      });
      // End send email

      console.log(
        `Email Token for ${finalUser.emailAddress}: ${emailToken} and id is ${
          finalUser.id
        }`
      );
      console.log(
        `Link is http://${clientServer}/registration/${
          finalUser.id
        }/${emailToken}`
      );
      return exits.success({
        message: "User successfully created. Please verify email to activate."
      });
    } else {
